...
function CountUp({ target, suffix = "" }: { target: number; suffix?: string }) {
  ...
  return (
    <p ref={ref} className="font-extrabold text-2xl md:text-[30px] text-azul">
      {count > 0 ? (target === 100 ? `${count}%` : `+${count}`) : "0"}
      {suffix}
    </p>
  );
}
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden">
      ...
      <div
        ...
        style={{ background: "linear-gradient(to right, rgba(5,18,36,0.93) 48%, rgba(5,18,36,0.25) 100%)" }}
      />
      ...
      <div className="relative z-10 h-full flex flex-col justify-end max-w-7xl mx-auto px-6 sm:px-10 lg:px-12 pb-16 md:pb-20">
        ...
        >
          ...
          >
            Coleta, Transporte e Destinação de Resíduos.
          </motion.p>
          ...
          >
            <a
              ...
              className="inline-flex items-center gap-2 bg-lime text-[#0a1a04] font-bold text-sm px-6 py-3 rounded-lg hover:scale-[1.02] transition-transform animate-[pulse_3s_ease-in-out_infinite] hover:animate-none shadow-[0_0_20px_rgba(141,198,63,0.3)]"
            >
              Solicitar Diagnóstico Gratuito
            </a>
            <a
              href="#servicos"
              className="inline-flex items-center gap-2 border text-sm font-medium px-6 py-3 rounded-lg text-white hover:bg-white/10 transition-colors"
          ...
          >
            {[
              ...
              >
                ...
                <p className="text-[11px] uppercase tracking-wider" style={{ color: "rgba(255,255,255,0.45)" }}>
                  {c.label}
                </p>
              </motion.div>
            ))}
