      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {items.map((item, i) => (
            ...
            >
              <p className="font-syne font-extrabold text-[26px] text-petrol mb-1">{item.num}</p>
              <p className="font-dm font-bold text-sm text-petrol mb-2">{item.title}</p>
              <p className="font-dm text-xs text-[#889]">{item.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
